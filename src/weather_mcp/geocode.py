...
async def _nominatim_call(
    client: httpx.AsyncClient, params: dict, origin_address: str
) -> Coordinates:
    """Fetches the geographical coordinates for the given address using the Nominatim API."""
    ...
    if count == 0:
        raise LocationNotFoundError(
            f"No records found for {origin_address}.  Check the spelling of the city/state this is an exact match only response."
        )
    if count == 1:
