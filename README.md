...
- **`scripts/seed_db.py`** — loads the checked-in golden fixture (`scripts/test_data.json`) into the DB, with two modes: `strict` (reset + fixture only, for reproducible eval runs) and `drift` (fixture as a floor, plus the 10 most recent live discussions per office layered on top). Ingestion (`rag/ingest.py`) dedupes near-identical chunks — NWS reissues an AFD every few hours and often repeats a section verbatim, which otherwise inflates the corpus with copies.
- **`scripts/build_test_data.py`** — regenerates `scripts/test_data.json` from live NWS data (one discussion per office per available calendar day, ~6-7 days back — the most `api.weather.gov`'s product-listing endpoint exposes). **`scripts/recall_at_k.py`** — a quantified recall@k retrieval benchmark against the live corpus. **`scripts/audit_chunking.py`** — a one-off (not CI-wired) chunking-quality audit that samples real discussions, runs the actual chunker, and gets a bounded LLM judge score; it found and fixed two real chunking bugs, one a silent content-loss regex bug — see [`tests/eval/FINDINGS_RAG.md`](tests/eval/FINDINGS_RAG.md).
