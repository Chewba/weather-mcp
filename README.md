...
- **`scripts/build_test_data.py`** — regenerates `scripts/test_data.json` from live NWS data (one discussion per office per available calendar day, ~6-7 days back — the most `api.weather.gov`'s product-listing endpoint exposes). **`scripts/recall_at_k.py`** — a quantified recall@k retrieval benchmark against the live corpus.
